...
function App(props) {
  ...
  const saveBtn = () => {
    ...
    const star_data = Object.entries(starChanges).map(
      ([star_id, { location, elevation }]) => {
        return {
          id: star_id,
    ...
    const scan_data = Object.entries(scanChanges).map(
      ([scan_id, { location }]) => {
        return {
          id: scan_id,
  ...
  const processMoreSites = (sites, sitesCache, map) => {
    ...
    Object.entries(sitesCache).forEach(([key, site]) => {
      if (!cachedMarkers.current[key]) {
        ...
        const marker = getStockMarker(
          invertTuple(site.location),
          color,
          props.static_root + "/M3.png"
        );
  ...
  const processMoreStars = (stars, starsCache, map) => {
    ...
    Object.entries(starsCache).forEach(([key, star]) => {
      if (!cachedMarkers2.current[key]) {
        ...
        const marker = getImagedMarker(
          invertTuple(star.location),
          props.static_root + "/pin_star_lq.png"
        );
  ...
  const loadSiteData = (page, bounds, othermap) => {
    ...
    if (!page) {
      ...
      Object.values(cachedMarkers2.current).forEach((marker) =>
        marker.remove()
      );
      cachedMarkers2.current = {};

      Object.values(cachedMarkers3.current).forEach((marker) =>
        marker.remove()
      );
  ...
  const mapLoader = html` <div id="maploader" style=${STYLES.mapLoader}>
    <div
  ...
  const typeSelector = html` <div
    id="typeselector"
    style=${STYLES.typeSelector}
    class="map-type-selector"
  >
    <div
      class="card d-flex flex-column align-items-start justify-content-start p-3"
    >
      <h5>Type of sites to include:</h5>

      ${["geo-tag", "band", "hunt", "scans"].map(
        (type, idx) => html`
          <div class="d-flex flex-row align-items-center justify-content-start">
            ...
            />
            <label for="${type}" class="map-type-selector-label"
  ...
  const pendingChanges =
    Object.keys(siteChanges).length +
      Object.keys(starChanges).length +
      Object.keys(scanChanges).length >
    0;

  return html` <div class="d-flex flex-row m-2">
      <div class="d-flex flex-column align-items-center">
        <a class="btn btn-primary" href="../../"
          ><i class="fas fa-arrow-left mr-1" /> Go back
        </a>
      </div>
      <div
        class="d-flex flex-row flex-fill align-items-center justify-content-center"
      >
        <h4 class="ml4 text-truncate" style="line-height: 36px;">
          Sites Location Editor
        </h4>
    ...
    <div id="map" style=${STYLES.map}>
      ...
      <div id="mapcontrols" style=${STYLES.mapControls}>
        <a
          class=${STYLES.buttonsClass}
          onClick=${() => setSatMapStyle(!satMapStyle)}
          ><i class="fa-solid fa-layer-group white"
        /></a>
        <a
          class="${STYLES.buttonsClass} mt4 ${pendingChanges
            ? "bg-green"
            : "bg-gray"}"
          onClick=${saveBtn}
          ><i class="fa-solid fa-floppy-disk black "
        /></a>
        <a
          class="${STYLES.buttonsClass} ${pendingChanges
            ? "bg-red"
            : "bg-gray"}"
          onClick=${restoreBtn}
          ><i class="fa-solid fa-rotate-left black "
        /></a>
      </div>
      ${dataLoading && mapLoader} ${typeSelector}
    </div>`;
}

render(
  html` <${App} ...${window.app_vars} />`,
  document.getElementById("preactapp")
);
