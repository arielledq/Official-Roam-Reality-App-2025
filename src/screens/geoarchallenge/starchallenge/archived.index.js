...
const StarChallenge = ({}) => {
  ...
  const ARScreen = props => {
    ...
    const [allStarsObj, setAllStarsObj] = useState(
      props?.arSceneNavigator.viroAppProps.allStarsObj
    );
    ...
    const [compassHeading, setCompassHeading] = useState(
    ...
    const [starShouldVisible, setStarShouldVisible] = useState(
    ...
    return (
      <ViroARScene onTrackingUpdated={onInitialized}>
        ...
        {allStarsObj.map(starObjE => {
          ...
          const scale = [
            challengeObjParameters?.scale_object
          ...
          for (j = 0; j < starObjE.star_location.coordinates.length; j++) {
            ...
            const starShouldVisibleNow = isLocationPointWithinRadius(
              currentLocation,
              starPoint,
              Number(starObjE?.visibility_radius)
            );
              ...
              ViroMaterials.createMaterials({
                ...
                mat: {
                  ...
                  bloomThreshold: challengeObjParameters
                    ...
                    : 0.5,
                  diffuseColor: challengeObjParameters
                    ? challengeObjParameters?.diffuse_text_color
                    : "#fff",
              ...
              return (
                ...
                >
                  <Viro3DObject
                    ...
                    position={[
                      challengeObjParameters?.positionX
                        ? Number(challengeObjParameters?.positionX)
                        : 0,
                      challengeObjParameters?.positionY
                        ? Number(challengeObjParameters?.positionY)
                        : -5,
                      challengeObjParameters?.positionZ
      ...
      try {
        if (arrayPoints.length > 0) {
          ...
          const starShouldVisibleNow = isLocationPointWithinRadius(
            position.coords,
            neareastPoint,
            Number(neareastPoint.starObj.visibility_radius)
          );
          ...
          if (
            neareastPoint.latitude == this.state.nearestPoint?.latitude &&
            neareastPoint.longitude == this.state.nearestPoint?.longitude
          ) {
            this.setState({
      ...
      return (
        <View style={{ flex: 1 }}>
          <BackgroundWithImage style={_styles.mainContainer}>
            <AppHeader
              centerComponent={{
                text: this.state.starShouldVisible
                  ? "You found a star!"
                ...
                <View style={{ flexDirection: "row" }}>
                  ...
                  <View>
                    ...
                    <Text style={_styles.arrivedText}>
                      {this.state.collectedStars.length} / {this.state.starsCount}
                    </Text>
                  </View>
                </View>
                <View style={{ flexDirection: "row" }}>
              ...
              >
                  ...
                  <View style={{ flexDirection: "row", flex: 1 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      ...
                      {this.state.allStarsCollected ? (
                        ...
                        <Text style={_styles.arrivedText}>
                          {this.state.starShouldVisible
                            ? "You found a star!"
                            : `${this.state.distanceInFeet} feet away`}
                        </Text>
                      )}
                    </View>
                  </View>
                ...
                <View style={{ flexDirection: "row" }}>
                  ...
                  <Text style={_styles.infoText}>
                    The dot pulsates quicker and the chime beeps faster when you get closer to a
                    Star. You can mute the sound by clicking on the speaker.
                  </Text>
                </View>
              </View>
            </View>
