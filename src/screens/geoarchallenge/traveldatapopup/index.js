...
const TravelDataPopUp = ({ currentLocation }) => {
  ...
  const placeARObjects = () => {
    ...
    const ARTags = places.map(item => {
      ...
      const distance = getLocationDistance(currentLocation, {
        latitude: item.lat,
        longitude: item.lng,
      })
      return (
        <ViroNode
          key={item.id}
          scale={[isNaN(scale) ? 0 : scale, isNaN(scale) ? 0 : scale, isNaN(scale) ? 0 : scale]}
          rotation={[0, 0, 0]}
