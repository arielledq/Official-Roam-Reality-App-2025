    ...
    return (
        <View style={{ width: '100%', maxHeight: '85%' }}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems:'center', marginTop: 25 }}>
                <Text style={{ fontSize: 18, fontWeight: 'bold', color: 'white' }}>Hidden Gems Available</Text>
                <AppButton
                    ...
                    return (
                        <View key={site.id} style={{ marginBottom: 15 }}>
                            <TouchableOpacity
                                ...
                                style={{ backgroundColor: '#2C2D3F', borderRadius: 10, padding: 12, flexDirection: 'row', alignItems: 'center' }}
                            >
                                <Image source={site.backgroundImage} style={{ width: 50, height: 50, borderRadius: 6, marginRight: 10 }} />
                                <View style={{ flex: 1 }}>
                                    <Text style={{ color: 'white', fontSize: 16, fontWeight: 'bold' }}>{site.location}</Text>
                                ...
                                <View style={{ marginTop: 10 }}>
                                    {site.challenges.map(challenge => (
                                        <TouchableOpacity
                                            ...
                                            style={{ backgroundColor: '#1E1F30', borderRadius: 10, flexDirection: 'row', alignItems: 'center', padding: 12, marginBottom: 10 }}
                                        >
                                            <View style={{ backgroundColor: '#7A32F4', borderRadius: 5, padding: 6, alignItems: 'center', justifyContent: 'center', width: 50 }}>
                                                <Text style={{ color: 'white', fontSize: 14, fontWeight: 'bold' }}>{challenge.points}</Text>
                                                <Text style={{ color: 'white', fontSize: 10 }}>Points</Text>
                                            </View>
                                            <View style={{ flex: 1, marginLeft: 10 }}>
                                                <Text style={{ color: 'white', fontSize: 14, fontWeight: 'bold' }}>{challenge.title}</Text>
                                                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 2 }}>
                                                    <Text style={{ color: '#C881F0', fontSize: 12 }}>{challenge.captures.current}/{challenge.captures.total} Captures</Text>
                                                    <Text style={{ color: '#C881F0', fontSize: 12, marginLeft: 10 }}>⏱ {challenge.cooldownHours}Hrs Cooldown</Text>
                                                </View>
                                            </View>
                                            <Image source={challenge.logo} style={{ width: 40, height: 40, borderRadius: 20 }} />
                                        </TouchableOpacity>
                                    ))}
                                </View>
                            )}
