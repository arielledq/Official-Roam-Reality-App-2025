...
const ARModeModal: React.FC<ShareToSocialsModalProps> = ({
  ...
  const filteredSponsors = (() => {
    if (selectedDropdownValue) {
      ...
      return SPONSORS_DATA.filter(sponsor => sponsor.id === selectedDropdownValue.toString());
    } else if (selectedSponsors.length > 0) {
      // Mostrar los seleccionados manualmente
      return SPONSORS_DATA.filter(sponsor => selectedSponsors.includes(sponsor.id));
    } else {
      // No hay selección: mostrar todos
